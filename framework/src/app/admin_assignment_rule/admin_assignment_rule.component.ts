...
@Component({
  selector: 'app-admin_assignment_rule',
  templateUrl: './admin_assignment_rule.component.html',
  styleUrls: ['./admin_assignment_rule.component.css']
})
export class Admin_assignment_ruleComponent implements OnInit {
  ...
  // displayedColumns: string[] = ['select', 'position', 'name', 'weight', 'symbol', 'errorcode', 'assigned', 'Due', 'status', 'Actions'];
  // dataSource = new MatTableDataSource<PeriodicElement>(ELEMENT_DATA);
  // selection = new SelectionModel<PeriodicElement>(true, []);
  ...
  this.columns = [
    ...
    { prop: 'name', name: 'Rule Name', canAutoResize: true, resizable: false },
    { prop: 'version', name: 'Version', canAutoResize: true, resizable: false },
    //{ prop: '', name: 'Queue Id', canAutoResize: true, resizable: false },
  ...
  openDialogconfirm(row): void {
    ...
    this._Mainservice.deleteassignmentrule(result).subscribe(() => {
      console.log(result);
  ...
  removeFile(row) {
    ...
    this._Mainservice.deleteassignmentrule(row).subscribe(() => {
  ...
  openDialog(row): void {
    console.log("enter")
    console.log(row)
    const dialogRef = this.dialog.open(Create_task_assignment_ruleComponent, {
      width: '600px',
      data: row
    });
