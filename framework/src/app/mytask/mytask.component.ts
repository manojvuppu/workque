...
export class MytaskComponent implements OnInit, AfterViewInit {
  ...
  filterobject = {
    ...
    hidecolums: [
      ...
      { name: 'Task Name', checked: true },
      { name: 'Priority', checked: true },
      { name: 'Queue Id', checked: true },
      { name: 'Error Code', checked: true },
      { name: 'Assigned To', checked: true },
      { name: 'Assigned Id', checked: true },
      { name: 'Created Date', checked: true },
      { name: 'Due At' , checked: true }
    ]
  }
  ...
  ngOnInit() {
    ...
    this._Mainservice.getfilter().subscribe((data: any) => {
      ...
      this.taskPriorityList = data.prioritycolumns;
      //console.log(this.filterobject.prioritycolums);
      this.taskStatusList = data.statuscolumns;
      this.hidecolums = this.filterobject.hidecolums;
    });
  ...
  onviewSelect(i) {
    ...
    this.viewname = this.viewslist[i].name;
    this.taskPriorityList = this.viewslist[i].selectedFilterList.prioritycolums;
    this.taskStatusList = this.viewslist[i].selectedFilterList.statuscolums;
    this.hidecolums = this.viewslist[i].selectedFilterList.Hiddencolumn;

  }

  applyfilter(priority, status, hide) {
    console.log(priority, status, hide);
  }
  ...
  openDialog2(taskPriorityList, taskStatusList, allColumns): void {
    console.log(taskPriorityList, taskStatusList);
    let obj = {
